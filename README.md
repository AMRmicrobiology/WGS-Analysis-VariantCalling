...
- Ensure [Java 8](https://github.com/winterbe/java8-tutorial) or later version is installed.
...
*ASSEMBLE*
```
nextflow run main.nf --mode assemble --input "/path/to/data/*_{1,2}.fastq.gz" --organism "Staphylococcus aureus" --mrsa <true> -profile <docker/singularity/conda>
```
*DE NOVO*

```
nextflow run main.nf --mode novo --input "/path/to/data/*_{1,2}.fastq.gz" --genome_name_db "Acinetobacter_baumanii" --wildtype_code "Pa01WT" -profile <docker/singularity/conda>
```

*REFERENCE GENOME*
```
nextflow run main.nf --mode reference --input "/path/to/data/*_{1,2}.fastq.gz" --personal_ref "/path/to/bacterial_genome.fasta" --custom_gff3 "/path/to/bacterial_genome.gff3" -profile <docker/singularity/conda>
```
...
│--organism     TEXT    To be used by Ariba and Abricate. List on ./organisms_list.txt [required]         │
│                       Abricate uses specific databases for Escherichia coli and Klebsiella pneumoniae   │
│-profile       TEXT    Selection of execution profile (docker, singularity or conda) [required]          │
│                                                                                                         │
│  Pipeline specific                                                                                      │
│--genome_name_db   TEXT    (--mode novo) Name of the organism and SnpEFF database [required]             │
│--wildtype_code    TEXT    (--mode novo) Defines the sample that will be taken as reference [required]   │
│--personal_ref     TEXT    (--mode reference) Input reference genome in FASTA file [required]            │
╰─────────────────────────────────────────────────────────────────────────────────────────────────────────╯
╭─ Additional Options ────────────────────────────────────────────────────────────────────────────────────╮
│--outdir           PATH    Directory to write the output [default: out]                                  │
│-w                 PATH    Path to the work directory where temporary files will be written              │
│                           [default: ./work ]                                                            │
│--mrsa             BOOLEAN (--mode assemble)   For *Staphylococcus aureus* assemblies                    │
│                           It performs the spaTyper and sccmec software analysis [default: false]        │
│--custom_gff3      TEXT    (--mode reference) Input .gff3 file to be used as annotation                  │
...
│--length_required  INTERGER    Reads shorter than length_required will be discarded [default: 50]        │
╰─────────────────────────────────────────────────────────────────────────────────────────────────────────╯
╭─ Filter Options ────────────────────────────────────────────────────────────────────────────────────────╮
│--qual_snp         TEXT    One or more expressions used with INFO fields to quality filter SNPs.         │
│                           [default: "QUAL < 50.0 || MQ < 25.0 || DP < 30"]                              │
│--qual_indel       TEXT    One or more expressions used with INFO fields to quality filter INDELs.       │
│                           [default: "QUAL < 200.0 || MQ < 25.0 || DP < 30"]                             │
╰─────────────────────────────────────────────────────────────────────────────────────────────────────────╯
```

>[!NOTE]
The filtering abreviations refer to &rarr; QUAL: A confidence measure of the variant; MQ: Mapping quality; DP: Filtered reads that support each of the reported alleles (depth). For more information refer to [GATK](https://gatk.broadinstitute.org/hc/en-us/articles/360035890471-Hard-filtering-germline-short-variants).
